...
import { Car, Clock, Film, Search, Layers } from "lucide-react";
...
export default function Home() {
  ...
  function reset() {
    ...
    setSelectedGenres([]); setMaxMovies(2); setError(""); setActiveTab(1);
  }

  const resolvedMinutes = mode === "route" ? routeInfo?.durationMinutes : parseInt(manualMinutes) || null;

  const tabLabels: Record<number, string> = { 1: "1 película", 2: "2 películas", 3: "3 películas", 4: "4 películas", 5: "5 películas" };
    ...
    <main style={{ minHeight: "100vh", padding: "0 0 80px" }}>
      ...
      <div style={{ background: "linear-gradient(180deg, #0f0f18 0%, var(--bg) 100%)", borderBottom: "1px solid var(--border)", padding: "48px 24px 40px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "1rem", maxWidth: 480, margin: "0 auto", lineHeight: 1.6 }}>
          Ingresá tu viaje y encontrá la combinación de películas perfecta para aprovechar cada minuto.
        </p>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 20px 0" }}>
        ...
        <section style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: "flex", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 4, gap: 4, marginBottom: 16, width: "fit-content" }}>
            {([["route", "Calcular ruta", Car], ["manual", "Ingresar minutos", Clock]] as const).map(([m, label, Icon]) => (
              <button key={m} onClick={() => { setMode(m); setSearched(false); setError(""); }} style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", borderRadius: 8, border: "none", background: mode === m ? "var(--accent)" : "transparent", color: mode === m ? "var(--bg)" : "var(--text-muted)", fontFamily: "var(--font-body)", fontSize: "0.88rem", cursor: "pointer", transition: "all 0.2s", fontWeight: mode === m ? 500 : 400 }}>
                <Icon size={14} />{label}
              </button>
            ))}
          </div>
          ...
          {mode === "route" ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <input
                ...
                style={{ width: "100%", background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: "var(--radius)", color: "var(--text)", fontFamily: "var(--font-body)", fontSize: "0.95rem", padding: "12px 16px", outline: "none" }}
              />
              <input
                value={destText}
                onChange={(e) => setDestText(e.target.value)}
                placeholder="Destino (ej: Mar del Plata)"
                style={{ width: "100%", background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: "var(--radius)", color: "var(--text)", fontFamily: "var(--font-body)", fontSize: "0.95rem", padding: "12px 16px", outline: "none" }}
              />
            </div>
          ) : (
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              {manualMinutes && parseInt(manualMinutes) >= 30 && (
                <span style={{ color: "var(--accent)", fontSize: "0.85rem" }}>≈ {Math.floor(parseInt(manualMinutes) / 60)}h {parseInt(manualMinutes) % 60}min</span>
              )}
            </div>
          )}
        </section>
        ...
        <section style={{ marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--surface2)", color: "var(--text-muted)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display)", fontSize: "1rem" }}>2</div>
            <h2 style={{ fontFamily: "var(--font-display)", fontSize: "1.4rem", letterSpacing: "0.05em", color: "var(--text-muted)" }}>PREFERENCIAS</h2>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: "0.74rem", color: "var(--text-muted)", marginBottom: 8, opacity: 0.7 }}>Las películas pueden pertenecer a cualquiera de los géneros elegidos.</p>
        ...
        <div style={{ marginBottom: 32, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          <button
            ...
            disabled={loading}
            style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 28px", borderRadius: "var(--radius)", background: "var(--accent)", border: "none", color: "var(--bg)", fontFamily: "var(--font-display)", fontSize: "1.1rem", letterSpacing: "0.08em", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1, transition: "all 0.2s" }}>
            <Search size={18} />{loading ? "BUSCANDO..." : "BUSCAR PELÍCULAS"}
          </button>
          {searched && (
            <button onClick={reset} style={{ display: "flex", alignItems: "center", gap: 8, padding: "14px 20px", borderRadius: "var(--radius)", background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text-muted)", fontFamily: "var(--font-display)", fontSize: "1rem", letterSpacing: "0.08em", cursor: "pointer" }}>
              ↺ NUEVA BÚSQUEDA
            </button>
          )}
          {error && <p style={{ color: "#ef4444", fontSize: "0.88rem", width: "100%" }}>{error}</p>}
        </div>

        {/* Resultado de ruta */}
        {routeInfo && (
          <div style={{ display: "flex", gap: 20, padding: "14px 18px", background: "var(--surface)", border: "1px solid var(--accent)", borderRadius: "var(--radius)", marginBottom: 24, flexWrap: "wrap" }}>
            <div><span style={{ fontSize: "0.75rem", color: "var(--text-muted)", display: "block" }}>RUTA</span><span style={{ fontSize: "0.9rem", color: "var(--text)" }}>{routeInfo.originLabel} → {routeInfo.destinationLabel}</span></div>
        ...
        {searched && (
          <section>
            ...
            {/* Pestañas */}
            <div style={{ display: "flex", marginBottom: 20, borderBottom: "1px solid var(--border)", overflowX: "auto" }}>
              {Array.from({ length: maxMovies }, (_, i) => i + 1).map((n) => {
                ...
                return (
                  <button key={n} onClick={() => setActiveTab(n)} style={{ padding: "10px 16px", border: "none", borderBottom: isActive ? "2px solid var(--accent)" : "2px solid transparent", background: "transparent", color: isActive ? "var(--accent)" : "var(--text-muted)", fontFamily: "var(--font-body)", fontSize: "0.85rem", fontWeight: isActive ? 500 : 400, cursor: "pointer", transition: "all 0.2s", marginBottom: -1, whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 6 }}>
                    {tabLabels[n]}
                    <span style={{ fontSize: "0.7rem", background: isActive ? "var(--accent)" : "var(--surface2)", color: isActive ? "var(--bg)" : "var(--text-muted)", borderRadius: 10, padding: "1px 7px", transition: "all 0.2s" }}>{count}</span>
                  </button>
                );
              })}
            </div>
            ...
            {(bySize[activeTab] || []).length === 0 ? (
              <div style={{ padding: "40px 24px", textAlign: "center", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius)" }}>
                <p style={{ color: "var(--text-muted)", fontSize: "0.95rem" }}>No encontramos combinaciones de {activeTab} película{activeTab > 1 ? "s" : ""} para este tiempo.</p>
              </div>
            ) : (
      ...
      <div style={{ textAlign: "center", marginTop: 60, padding: "20px", borderTop: "1px solid var(--border)", color: "var(--text-muted)", fontSize: "0.78rem" }}>
        Powered by <a href="https://www.themoviedb.org" target="_blank" rel="noreferrer" style={{ color: "var(--accent)", textDecoration: "none" }}>TMDB</a>{" & "}<a href="https://openrouteservice.org" target="_blank" rel="noreferrer" style={{ color: "var(--accent)", textDecoration: "none" }}>OpenRouteService</a>
      </div>
    </main>
  );
}